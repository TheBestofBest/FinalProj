...
const MailboxSend: React.FC = () => {
  ...
  const fetchSendMails = async () => {
    try {
      const memberId = memberData.memberId;
      const response = await api.get(`/api/v1/mailboxes/${memberId}/mails/sent`);
      console.log(response);

      if (response.data.isSuccess) {
        const sendMails = response.data.data;
        if (sendMails) {
          console.log("============================================================");
          console.log(sendMails); // 전체 메일 목록 확인
          console.log("보낸 메일 목록(allmails 상태):", sendMails); // 상태 값 확인
          setSendMails(sendMails);
        }
      } else {
        console.error("보낸 메일 불러오기 실패:", response.data.message);
      }
    } catch (error) {
      console.error("보낸 메일 불러오기 에러:", error);
    }
  };
