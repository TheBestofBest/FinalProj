...
const MailboxReference: React.FC = () => {
  const queryClient = useQueryClient();
  const memberData: any = queryClient.getQueryData(["member"]);
  const [totalPages, setTotalPages] = useState(0);
  const [isChecked, setIsChecked] = useState<boolean>(false);
  const [refMails, setRefMails] = useState({});

  useEffect(() => {
    fetchRefMails();
  }, []);

  const fetchRefMails = async () => {
    try {
      const memberId = memberData.memberId;
      const response = await api.get(`/api/v1/mailboxes/${memberId}/mails/referenced`);
      console.log(response);

      if (response.data.isSuccess) {
        const refMails = response.data.data;
        if (refMails) {
          console.log("============================================================");
          console.log(refMails); // 전체 메일 목록 확인
          console.log("참조 메일 목록(refMails 상태):", refMails); // 상태 값 확인
          setRefMails(refMails);
        }
      } else {
...
export default MailboxReference;
