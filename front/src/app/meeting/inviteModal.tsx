...
const InviteModal = ({ closeModal }: any) => {
    ...
    const invite = async (selectedName: string | null) => {
        if (params.id != null) {
            const response = await api.patch(`/api/v1/meetings/${params.id}/invite`, { username: selectedName });
    ...
    return (
        <div className="h-screen z-50 w-full fixed left-0 top-0 flex justify-center items-center bg-black bg-opacity-70 text-center">
